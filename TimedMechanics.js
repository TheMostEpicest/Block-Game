...
function timer() {
    ...
    while (Date.getTime() - startTime > timerValue) {
        document.getElementById("timer").innerHTML = timerValue - (Date.getTime() - startTime);
    }
...
function setWinScore() {
    if (parseInt(document.getElementById("enterWin").value) > 0 && parseInt(document.getElementById("enterWin").value) > scoreBlu && parseInt(document.getElementById("enterWin").value) > scoreRed) {
        winScore = parseInt(document.getElementById("enterWin").value, 10);
...
function chooseRandomWalls() {
    for (i = 0; i < document.getElementsByClassName("RandomWall").length; i++) {
        if (Math.ceil(Math.random() * 5) === 1) {
            document.getElementsByClassName("RandomWall")[i].classList.add("wall");
        }
    }
}
